...
class ResidualAttentionNetwork():
    ...
    def __init__(self, input_shape, output_size, mask_activation, p=1, t=2, r=1):
        ...
        self.t = t
        self.r = r
#         self.filter_dic = {'s1': [64,64,128],
#                            's2': [64,64,128],
#                            's3': [64,64,128],
#                            'se': [64,64,128]}
        self.filter_dic = {'s1': [64,64,256],
                           's2': [128,128,512],
                           's3': [256,256,1024],
                           'se': [512,512,2048]}
    ...
    def Attention_92(self):
        ...
        fully_connected_layer_1 = Dense(128, activation='softmax')(flatten)
        dropout_layer_1 = Dropout(0.5)(fully_connected_layer_1)
        fully_connected_layer_2 = Dense(128, activation='softmax')(dropout_layer_1)
        dropout_layer_2 = Dropout(0.5)(fully_connected_layer_2)
        
        output = Dense(self.output_size, activation='softmax')(dropout_layer_2)
...
#         nal_unit = self.NaiveAttentionLearning(trunk_unit, mask_unit)
#         # p iterations of ResUnit
#         for _ in range(self.p):
#             nal_unit = self.ResidualUnit(nal_unit, filters=filters)      
    ...
    def AttentionResidualLearning(self, trunk_unit, mask_unit):
        output = Multiply()([trunk_unit, mask_unit])
        output = Add()([output, mask_unit])
        
        return output
        
        
    def NaiveAttentionLearning(self, trunk_unit, mask_unit):
        output = Multiply()([trunk_unit, mask_unit])
        return output
    ...
    def Attention_56_ResNeXt(self):
        input_data = Input(shape=self.input_shape)
        padded_data = ZeroPadding2D((4,4))(input_data)
        
        conv_1 = Conv2D(filters=64, kernel_size=(7,7), strides=(2,2), padding='same')(padded_data)
        max_pool_1 = MaxPool2D(pool_size=(3,3), strides=(2,2), padding='same')(conv_1)
        
        filters_s1 = [128,128,256]
        res_unit_1 = self.ResNeXtUnit(max_pool_1, filters=filters_s1)
        am_unit_1 = self.AttentionModule(res_unit_1, filters=filters_s1)
        
        filters_s2 = [256,256,512]
        res_unit_2 = self.ResNeXtUnit(am_unit_1, filters=filters_s2)
        am_unit_2 = self.AttentionModule(res_unit_2, filters=filters_s2)
        
        filters_s3 = [512,512,1024]
        res_unit_3 = self.ResNeXtUnit(am_unit_2, filters=filters_s3)
        am_unit_3 = self.AttentionModule(res_unit_3, filters=filters_s3)
        ...
        filters_ending = [1024,1024,2048]
        for i in range(3):
            res_unit_ending = self.ResNeXtUnit(res_unit_ending, filters=filters_ending)
    ...
    def ResNeXtUnit(self, x, filters):
        ...
        for _ in range(32):
            batch_norm_1 = BatchNormalization()(x)
            activation_1 = Activation('relu')(batch_norm_1)
            conv_1 = Conv2D(filters=filter1, kernel_size=(1,1), padding='same')(activation_1)
